...
(defvar helm-ros--launchfile-actions '(("Open File" . helm-ros--open-file-action)
...
(defun helm-ros--roslaunch ()
  "Put the launch launchfile action first, an then start helm with the launchfile source."
  (interactive)
  (push '("Launch" . helm-ros--launch-launchfile) helm-ros--launchfile-actions)
  (cl-remove-duplicates helm-ros--launchfile-actions)
  (helm :sources helm-source-ros-launchfiles
        :buffer "*helm-roslaunch*")
  (push '("Open File" . helm-ros--open-file-action) helm-ros--launchfile-actions)
  (cl-remove-duplicates helm-ros--launchfile-actions))
...
(defun helm-ros--fetch-list-of-packages ()
  (helm-ros--list-of-command-output "rospack list"))
...
(defun helm-ros--list-of-package-names ()
  (mapcar (lambda (x)
            (let ((parsed-entry (helm-ros--parsed-rospack-entry x)))
              (car parsed-entry)))
          (helm-ros--fetch-list-of-packages)))
...
(defun helm-ros--launch-node (node)
  (interactive)
...
(defvar helm-source-ros-nodes
  (helm-build-sync-source "Nodes"
    ...
    :action '(("Run node" . (lambda (node) (helm-ros--launch-node node))))))

;;;###autoload
(defun helm-ros ()
  "Launches helm-ros with all available sources."
  (interactive)
